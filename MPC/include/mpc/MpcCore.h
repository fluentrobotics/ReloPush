...
// shortest-path (wrap-aware) yaw interpolation. Past the trajectory's END
// (rel_time >= the last waypoint's time, inclusive of exactly-at-the-end),
// pose/yaw/is_pushing stay clamped to the final waypoint but ref_vel is
// forced to 0.0: past the reference's end the plan wants the robot PARKED,
// and the last waypoint's own ref_vel is only a nominal per-leg speed label
// for the final motion segment -- meaningless, and actively harmful if fed
// to a downstream velocity target (e.g. the LAUNCH governor), once there is
// no more trajectory left to execute. The before-start clamp (rel_time <=
// the first waypoint's time) has no such special case and returns the
// first waypoint's own ref_vel as-is. Returns ok=false on an empty (or
// null) trajectory instead of dereferencing front()/back().
...
// ---------------------------------------------------------------------
// SCHEDULE CATCH-UP: behind-schedule velocity boost.
// ---------------------------------------------------------------------
//
// THE PROBLEM: the control loop tracks the reference PATH (position/
// heading) closely -- see MPCCostFunctor's position/lateral/heading
// residuals above -- but nothing reacts to falling behind the reference
// SCHEDULE specifically: a robot that is, say, 0.15m behind where the
// reference currently is (while still tracking the reference's SHAPE
// faithfully, just with a time lag) sees no extra pull to speed up, since
// the velocity-tracking residual (channel 4) only ever pulls v toward the
// CURRENT stage's nominal ref.ref_vel, never above it. Over a cruise leg
// this shows up as a persistent along-track gap between the actual robot
// and where the reference/GUI overlay currently is -- visible directly in
// mars_sim_viz as the solid (actual) robot trailing the dashed (reference)
// overlay.
//
// THE FIX: boosted_vr() raises (or lowers) the velocity TARGET fed into the
// MPC's own tracking cost -- it never overrides the solve's own decision
// the way the LAUNCH GOVERNOR does -- so the solver still owns the actual
// control decision, now just chasing a smarter target. main.cpp computes
// ONE scalar e_lag per tick (signed along-track position error, current
// estimated pose vs. the CURRENT reference pose -- see main.cpp's call
// site) and applies boosted_vr() to EVERY horizon stage's ref.ref_vel
// before hanging horizon_refs to solve(), so the whole horizon -- not just
// stage 0 -- reflects the boosted target.
//
// SIGN CONVENTION, AND WHY THERE IS NO EXTRA sign(vr) FACTOR: e_lag is
// (ref.x - est.x)*cos(ref.yaw) + (ref.y - est.y)*sin(ref.yaw) -- positive
// when the reference is further along +heading than the robot. For a
// FORWARD leg (vr_nominal > 0) that IS "behind": the robot needs to go
// faster in +heading, i.e. vr should INCREASE, so the correction should be
// +K*e_lag. For a REVERSE leg (vr_nominal < 0, translating along -heading),
// "behind" means the robot has translated LESS FAR along -heading than the
// reference has by now -- i.e. the robot's position is further along
// +heading than the reference's, which is e_lag < 0 under the very SAME
// formula (the reference, having reversed further, is now further along
// -heading -- i.e. less far along +heading -- than the robot). To catch up
// on a reverse leg the robot must go MORE NEGATIVE (reverse faster), i.e.
// the correction must ALSO be negative when e_lag < 0 -- which
// "correction = +K*e_lag" already provides directly, with NO sign(vr)
// factor: e_lag's own sign convention already flips the correct way
// between forward and reverse legs on its own. Multiplying by an EXTRA
// sign(vr) would double-flip the reverse case and push it the WRONG way
// (verified by mpc_unit_tests.cpp's dedicated reverse-leg catch-up case --
// this comment spells out the derivation rather than just asserting it
// precisely because that literal reading is the natural first guess and is
// wrong).
//
// vr_nominal == 0.0 (a hold/stop stage's stationary target) is NEVER
// boosted -- e_lag against a stationary reference is either meaningless
// (mid-hold) or would fight STOP-SNAP/trajectory-completion for no benefit
// (main.cpp separately also suppresses e_lag itself to 0.0 during/shortly
// after a REFERENCE STOP EVENT -- see that file -- this check is cheap
// belt-and-suspenders on top, not a substitute).
//
// CLAMP: the corrected magnitude is clamped to [0.5*|vr_nominal|,
// vmax_stage] -- catch-up is capped at this stage's existing push/non-push
// speed cap (never asks for more than clamp_sym already enforces inside the
// solve), and ease-off never drops below half the nominal reference speed
// even for an arbitrarily large "ahead" reading, so a noisy/extreme e_lag
// can only ever slow the robot to a still-forward-progressing crawl in the
// ORIGINAL direction, never stall or reverse it. Implemented as a signed
// clamp of vr_prime directly into [dir*0.5*|vr_nominal|, dir*vmax_stage]
// (the two endpoints ordered via min/max, since which one is numerically
// smaller flips between dir=+1 and dir=-1) rather than clamping
// |vr_prime| and reapplying dir's sign afterward -- the latter would
// incorrectly snap back UP to vmax for an extreme sign-flipping correction
// instead of correctly flooring at half nominal (also verified by a
// dedicated extreme-e_lag unit test case).
double boosted_vr(double vr_nominal, double e_lag, double K, double vmax_stage);
