...
RefState get_ref_state_at_time(double abs_time, double traj_start_time,
    ...
    if (rel_time >= points.back().time) {
        // Past the reference's END (inclusive of exactly-at-the-end): the
        // plan wants the robot PARKED here. Pose/yaw/is_pushing stay
        // clamped to the final waypoint as before, but ref_vel is forced
        // to 0.0 -- the waypoint's own ref_vel is only a per-leg nominal
        // speed label for the final motion segment, meaningless (and
        // actively harmful if fed to a downstream velocity target, e.g.
        // the LAUNCH governor's ref_indicates_motion check) once there is
        // no more trajectory left to execute. See this function's doc
        // comment in MpcCore.h.
        RefState out = ref_state_from_point(points.back());
        out.ref_vel = 0.0;
        return out;
    }
...
double boosted_vr(double vr_nominal, double e_lag, double K, double vmax_stage) {
    ...
    const double dir = (vr_nominal > 0.0) ? 1.0 : -1.0;
    // See this function's doc comment in MpcCore.h for the full derivation
    // of why this is +K*e_lag with NO extra sign(vr_nominal) factor: e_lag's
    // own sign convention already flips correctly between forward and
    // reverse legs on its own.
    const double vr_prime = vr_nominal + K * e_lag;
    const double cap = std::fabs(vmax_stage);
    const double floor_mag = 0.5 * std::fabs(vr_nominal);
    const double endpoint_floor = dir * floor_mag;
    const double endpoint_cap = dir * cap;
    // Signed clamp interval -- min/max resolves which endpoint is the
    // numeric lower bound, since that flips between dir=+1 (forward) and
    // dir=-1 (reverse). Clamping the SIGNED vr_prime (not |vr_prime|) is
    // what correctly floors an extreme "ahead" correction at half-nominal
    // instead of snapping it back up to vmax -- see the header doc comment.
    const double lo = std::min(endpoint_floor, endpoint_cap);
    const double hi = std::max(endpoint_floor, endpoint_cap);
    return std::clamp(vr_prime, lo, hi);
}
