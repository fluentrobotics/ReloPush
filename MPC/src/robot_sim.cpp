...
// mpc_controller already talks to (see MPC/src/main.cpp):
...
struct Options {
    ...
    bool deadband_enabled = false;              // FEATURE B: opt-in, default OFF (bit-identical when off).
    double min_moving_speed_override = 0.0;     // only used when has_min_moving_speed_override.
    bool has_min_moving_speed_override = false;
    double min_sustain_speed_override = 0.0;    // only used when has_min_sustain_speed_override.
    bool has_min_sustain_speed_override = false;
};
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--robot-name" && i + 1 < argc) {
            ...
            o.deadband_enabled = true;
        } else if (arg.rfind("--min-moving-speed=", 0) == 0) {
            const std::string val_str = arg.substr(std::string("--min-moving-speed=").size());
            try {
                size_t consumed = 0;
                o.min_moving_speed_override = std::stod(val_str, &consumed);
                if (consumed != val_str.size()) {
                    throw std::invalid_argument("trailing characters");
                }
                o.has_min_moving_speed_override = true;
                o.deadband_enabled = true;  // an explicit override implies enabling FEATURE B.
            } catch (const std::exception&) {
                std::cerr << "[SIM] Warning: invalid --min-moving-speed value '" << val_str
                           << "', ignoring override" << std::endl;
            }
        } else if (arg.rfind("--min-sustain-speed=", 0) == 0) {
            const std::string val_str = arg.substr(std::string("--min-sustain-speed=").size());
            try {
                size_t consumed = 0;
                o.min_sustain_speed_override = std::stod(val_str, &consumed);
                if (consumed != val_str.size()) {
                    throw std::invalid_argument("trailing characters");
                }
                o.has_min_sustain_speed_override = true;
                o.deadband_enabled = true;  // an explicit override implies enabling FEATURE B.
            } catch (const std::exception&) {
                std::cerr << "[SIM] Warning: invalid --min-sustain-speed value '" << val_str
                           << "', ignoring override" << std::endl;
            }
        } else {
...
int main(int argc, char** argv) {
    ...
    // FEATURE B: resolve the STARTING deadband enable flag/thresholds. An
    // explicit --min-moving-speed=/--min-sustain-speed= override wins over
    // the (possibly JSON-loaded) robot_spec value and also implies enabling
    // the feature (see parse_args) even without a bare --deadband.
    //
    // FEATURE B2: these three are deliberately NOT const -- a live
    // "/<robot>/sim_config" message (see the got_sim_config handling below)
    // can reconfigure any of them mid-run, taking effect on the very next
    // sim tick. "Applying 'deadband':1 uses the current min_moving/
    // min_sustain values (from spec/CLI unless previously overridden live)"
    // falls out naturally: the tick loop's `if (deadband_enabled)` branch
    // and its step_deadband() calls always read whatever these three
    // currently hold, so there is nothing else to wire up.
    bool deadband_enabled = opt.deadband_enabled;
    double effective_min_moving_speed = opt.has_min_moving_speed_override
                                             ? opt.min_moving_speed_override
                                             : robot_spec.min_moving_speed;
    double effective_min_sustain_speed = opt.has_min_sustain_speed_override
                                              ? opt.min_sustain_speed_override
                                              : robot_spec.min_sustain_speed;
    // BREAKAWAY REALISM (see RobotSpec::max_breakaway_accel / SimCore.h's
    // resolve_accel_clamp_ceiling doc comments): no CLI override flag today
    // (unlike the two thresholds above) -- only --robot-spec=<path> (or the
    // compiled default, 3.0) sets the STARTING value; FEATURE B2's live
    // sim_config channel below can still reconfigure it mid-run, same as the
    // two thresholds.
    double effective_max_breakaway_accel = robot_spec.max_breakaway_accel;
    ...
    std::cout << "[SIM " << opt.robot_name << "] Launching: cmd(SUB bind)=" << opt.cmd_endpoint
               ...
               << " steer_noise_sigma_pct=" << noise_model.steer_sigma_pct()
               << " deadband=" << (deadband_enabled ? "on" : "off");
    if (deadband_enabled) {
        std::cout << " min_moving_speed=" << effective_min_moving_speed
                   << " min_sustain_speed=" << effective_min_sustain_speed;
    }
    std::cout << std::endl;
    ...
    // live noise-reconfiguration channel, extended by FEATURE B2 to also
    // carry live deadband enable/threshold reconfiguration) on the SAME
    // socket -- a real VESC bridge never receives this topic, so it is
    // purely additive.
    ...
    const std::string telemetry_topic = "/" + opt.robot_name + "/telemetry";  // additive, always published.
    try {
    ...
    // FEATURE B state. v_cmd_equiv mirrors state.v when the feature is off
    // (kept in sync every tick below) so it can always be reported as
    // telemetry's "v_cmd" regardless of mode. deadband_moving is only
    // meaningful when deadband_enabled; left permanently true when the
    // feature is off (no stall modeling -- the robot is always considered
    // "moving").
    double v_cmd_equiv = opt.v0;
    bool deadband_moving = true;
    if (deadband_enabled) {
        deadband_moving = std::fabs(opt.v0) >= effective_min_moving_speed;
        state.v = deadband_moving ? opt.v0 : 0.0;  // resolve the initial effective velocity too.
    }
    ...
    while (!g_shutdown_requested) {
        ...
        if (got_ackermann) {
            ...
            if (cmd.ok) {
                // BREAKAWAY REALISM: while stalled (deadband on, not
                // currently moving), the RECEIVED command's accel is allowed
                // up to max_breakaway_accel instead of the nominal
                // max_accel -- see RobotSpec::max_breakaway_accel's doc
                // comment. This is what lets the LAUNCH GOVERNOR's KICK
                // (main.cpp) publish its truthful, briefly-large accel
                // without being clipped back down to max_accel here. A no-op
                // (returns max_accel exactly) whenever deadband is off.
                const double accel_ceiling = mpc::resolve_accel_clamp_ceiling(
                    deadband_enabled, deadband_moving, robot_spec.max_accel,
                    effective_max_breakaway_accel);
                mpc::ClampResult clamped = mpc::clamp_command(
                    cmd.accel, cmd.steering, accel_ceiling, robot_spec.max_steer);
                ...
                // Same BREAKAWAY REALISM ceiling as the receive-clamp just
                // above (re-derived fresh -- deadband_moving cannot have
                // changed between the two calls within this same tick) so a
                // noise-perturbed KICK command is not clipped back down to
                // max_accel here either.
                mpc::ClampResult applied = mpc::apply_command_noise(
                    last_cmd_accel, last_cmd_steering, pert, accel_ceiling,
        ...
        // FEATURE C / FEATURE B2: live noise-sigma AND deadband
        // reconfiguration via "/<robot>/sim_config" -- sim-only, a real VESC
        // bridge never receives this topic. Whatever this resolves to takes
        // effect starting the VERY NEXT sim tick (step 3 below reads
        // deadband_enabled/effective_min_moving_speed/
        // effective_min_sustain_speed fresh every tick; there is no
        // separate "apply" step and no state reset on either side of a
        // toggle -- see those variables' own doc comment above).
        if (got_sim_config) {
            mpc::SimConfigParseResult cfg = mpc::parse_sim_config_payload(
                last_sim_config_payload, noise_model.accel_sigma_pct(),
                noise_model.steer_sigma_pct(), deadband_enabled, effective_min_moving_speed,
                effective_min_sustain_speed, effective_max_breakaway_accel, robot_spec.max_accel);

            // Per-field malformed warnings: fired for EVERY field that was
            // PRESENT in the payload but failed to parse, independent of
            // whether any OTHER field in the SAME payload succeeded (same
            // per-field-independence discipline the noise channels already
            // have structurally -- see SimConfigParseResult's *_bad flags).
            if (cfg.accel_bad) {
                std::cerr << "[SIM " << opt.robot_name
            ...
            if (cfg.deadband_bad) {
                std::cerr << "[SIM " << opt.robot_name
                           << "] Warning: invalid 'deadband' in sim_config, retaining deadband="
                           << (deadband_enabled ? "on" : "off") << std::endl;
            }
            if (cfg.min_moving_speed_bad) {
                std::cerr << "[SIM " << opt.robot_name
                           << "] Warning: invalid 'min_moving_speed' in sim_config, retaining "
                              "min_moving_speed=" << effective_min_moving_speed << std::endl;
            }
            if (cfg.min_sustain_speed_bad) {
                std::cerr << "[SIM " << opt.robot_name
                           << "] Warning: invalid 'min_sustain_speed' in sim_config, retaining "
                              "min_sustain_speed=" << effective_min_sustain_speed << std::endl;
            }
            if (cfg.max_breakaway_accel_bad) {
                std::cerr << "[SIM " << opt.robot_name
                           << "] Warning: invalid 'max_breakaway_accel' in sim_config, retaining "
                              "max_breakaway_accel=" << effective_max_breakaway_accel << std::endl;
            }
            ...
            if (!cfg.ok) {
                ...
                // FEATURE B2: apply unconditionally (idempotent when
                // unchanged) but log exactly ONE line IFF something actually
                // changed -- avoids spamming a line every time the viz
                // re-publishes the same sim_config state. max_breakaway_accel
                // (OPTIONAL field) joins the same apply-and-log block.
                if (cfg.deadband_ok || cfg.min_moving_speed_ok || cfg.min_sustain_speed_ok ||
                    cfg.max_breakaway_accel_ok) {
                    const bool changed = (cfg.deadband_enabled != deadband_enabled) ||
                                          (cfg.min_moving_speed != effective_min_moving_speed) ||
                                          (cfg.min_sustain_speed != effective_min_sustain_speed) ||
                                          (cfg.max_breakaway_accel != effective_max_breakaway_accel);
                    deadband_enabled = cfg.deadband_enabled;
                    effective_min_moving_speed = cfg.min_moving_speed;
                    effective_min_sustain_speed = cfg.min_sustain_speed;
                    effective_max_breakaway_accel = cfg.max_breakaway_accel;
                    if (changed) {
                        std::cerr << "[SIM " << opt.robot_name << "] deadband set to "
                                   << (deadband_enabled ? "on" : "off") << " (min_moving="
                                   << effective_min_moving_speed
                                   << " min_sustain=" << effective_min_sustain_speed
                                   << " max_breakaway_accel=" << effective_max_breakaway_accel
                                   << ")" << std::endl;
                    }
                }
            }
        ...
        // "current_v" fed to resolve_command: v_cmd_equiv plays exactly the
        // role state.v always played here, deadband or not (see FEATURE B
        // doc comment in SimCore.h) -- with the feature off the two are kept
        // identical every tick below, so this is a no-op change in that case.
        const double current_v_for_resolve = deadband_enabled ? v_cmd_equiv : state.v;
        mpc::ResolvedCommand resolved = mpc::resolve_command(
            wd_engaged, current_v_for_resolve, last_applied_accel, resolve_steering_arg,
            robot_spec.max_accel);

        // 3. Integrate one fixed step. FEATURE B (deadband, opt-in): the
        //    command-equivalent velocity keeps integrating accel every tick
        //    exactly like state.v always has (modeling ERPM windup while
        //    stalled); a hysteresis "moving" flag gates what pose integration
        //    actually sees (mpc::step_deadband/integrate_pose_with_v -- see
        //    SimCore.h). With the feature OFF, this reduces EXACTLY to the
        //    pre-existing snap-then-rollout_step pipeline -- bit-identical.
        if (deadband_enabled) {
            // Same snap-to-zero rationale as the disabled branch below,
            // applied to v_cmd_equiv instead of state.v directly.
            if (resolved.snap_v_to_zero) {
                v_cmd_equiv = 0.0;
            }
            const double v_cmd_equiv_next = v_cmd_equiv + resolved.accel * dt_sim;
            const mpc::DeadbandStepResult db = mpc::step_deadband(
                v_cmd_equiv_next, deadband_moving, effective_min_moving_speed,
                effective_min_sustain_speed);
            deadband_moving = db.moving;
            state = mpc::integrate_pose_with_v(state, db.effective_v, resolved.steering,
                                                robot_spec.wheel_base, dt_sim);
            v_cmd_equiv = v_cmd_equiv_next;
        } else {
            // Watchdog-engaged + already-sub-eps residual v: force it to
            // exactly 0 before integrating. Without this, accel==0 alone
            // only HOLDS the residual (rollout_step's next.v = state.v +
            // a*dt with a==0 is a no-op on v), so a robot that idles here
            // (e.g. finished its own trajectory early while other robots are
            // still executing) drifts position at that residual's constant
            // rate for as long as it stays idle -- tens of seconds in
            // practice, easily enough to blow the final-position-error
            ...
            state = mpc::rollout_step(state, resolved.accel, resolved.steering,
                                       robot_spec.wheel_base, dt_sim);
            v_cmd_equiv = state.v;  // mirror state.v so telemetry's "v_cmd" stays meaningful.
        }
        ++tick_index;
        ...
        if (tick_index % loc_interval_ticks == 0) {
            ...
            // Telemetry (additive; see SimCore.h): published unconditionally
            // on the SAME PUB socket/rate -- never gated by --deadband,
            // since it only adds a new topic and never changes the
            // localization message above.
            mpc::TelemetrySample telemetry;
            telemetry.t = t;
            telemetry.v = state.v;
            telemetry.v_cmd = v_cmd_equiv;
            telemetry.steering = resolved.steering;
            telemetry.accel = resolved.accel;
            telemetry.moving = deadband_moving;
            telemetry.watchdog = wd_engaged;
            const std::string telemetry_payload = mpc::encode_telemetry_payload(telemetry);
            zmq::message_t telemetry_topic_msg(telemetry_topic.begin(), telemetry_topic.end());
            zmq::message_t telemetry_payload_msg(telemetry_payload.begin(), telemetry_payload.end());
            try {
                if (!loc_pub.send(telemetry_topic_msg, zmq::send_flags::sndmore)) {
                    std::cerr << "[SIM " << opt.robot_name
                               << "] Warning: failed to send telemetry topic frame" << std::endl;
                } else if (!loc_pub.send(telemetry_payload_msg, zmq::send_flags::none)) {
                    std::cerr << "[SIM " << opt.robot_name
                               << "] Warning: failed to send telemetry payload frame" << std::endl;
                }
            } catch (const zmq::error_t& ex) {
                std::cerr << "[SIM " << opt.robot_name << "] Warning: zmq telemetry send error ("
                           << ex.what() << ")" << std::endl;
            }
        }
