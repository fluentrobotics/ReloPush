...
//          trajectory (proving noise was actually active on both channels,
//          not silently inert -- see kMinTrajectoryDelta in main()).
//
...
// below). Mirrors MPCController/include/mpc/RobotSpec.h's compiled default
...
// MPCController/src/main.cpp's encodeAscii() -- the wire format
...
// tolerance check (see kMinTrajectoryDelta in main()).
...
int main() {
    ...
    // a coarse diagnostic, NOT a tolerance check.
    constexpr double kMinTrajectoryDelta = 0.005;
    ...
    if (!rep1.rows.empty() && !rep2.rows.empty()) {
        ...
        noise_was_active = trajectory_delta > kMinTrajectoryDelta;
    }
    ...
    std::cout << "\n=== Trajectory divergence (run1 vs run2) ===\n";
    std::cout << "  max pose delta = " << to_str(trajectory_delta) << " m (must be > "
              << kMinTrajectoryDelta << " to prove noise was active)\n";
    if (!noise_was_active) {
        std::cout << "  note: trajectories did not measurably diverge -- noise may not be active\n";
    }
